...
<template>
  <section class="flex flex-col w-full relative">
      ...
      <div class="flex flex-col w-[100vw] bg-white rounded-b-[10vw]">
        ...
        <div class="flex ml-[5vw] gap-5 items-center m-5 pb-5">
          <img v-if="pb.authStore.model.avatar" :src="avatar" class="aspect-square rounded-full w-[25vw] max-h-[150px] max-w-[150px]" style="object-fit: cover;"></img>
          <div v-else class="aspect-square rounded-full w-[25vw] max-h-[150px] max-w-[150px] bg-blue-900 flex justify-center content-center"><div class="h-fit text-white font-bold text-[7vw]">{{ pb.authStore.model.name[0].toUpperCase() }}</div></div>
